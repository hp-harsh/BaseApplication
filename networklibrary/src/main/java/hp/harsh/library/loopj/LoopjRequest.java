...
public class LoopjRequest {
    ...
    // Get all requested parameter and initiate API call
    public LoopjRequest(Activity activity, Method requestType, String url, Map<String, String> params, Map<String, String> headers, Object fileObject, int type, boolean isShowDialog, LoopjInterface objInterface) {
        this.mActivity = activity;
        this.mRequestType = requestType;
        this.mParams = params;
        this.mHeaders = headers;
        this.mFiles = fileObject;
        this.mIsShowProgressDialog = isShowDialog;
        this.mRequestId = type;
        this.mObjInterface = objInterface;
        mUrl = url;
    ...
    private synchronized void callWebApiRequest() {
        ...
        if (mFiles != null) {
            File sourceFile[] = (File[]) mFiles;
            for (int i = 1; i <= sourceFile.length; i++) {
                try {
                    requestParams.put("images" + i, sourceFile[i - 1]);
